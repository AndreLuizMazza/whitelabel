// src/lib/seo.js
import { resolveSeoDefaults } from "@/lib/branding/tenantContract.js";

/**
 * Atualiza <title> e metas de descrição/OG/Twitter.
 * Uso:
 *   setPageSEO({ title: 'Meu perfil', description: 'Edite seus dados...' })
 */
export function setPageSEO({ title, description, image, url } = {}) {
  if (typeof document === "undefined") return;

  if (title) document.title = title;

  const upsert = (selector, attrs = {}) => {
    let el = document.head.querySelector(selector);
    if (!el) {
      el = document.createElement("meta");
      if (selector.startsWith("meta[name=")) {
        el.setAttribute("name", selector.match(/meta\[name="([^"]+)"\]/)[1]);
      } else if (selector.startsWith("meta[property=")) {
        el.setAttribute(
          "property",
          selector.match(/meta\[property="([^"]+)"\]/)[1]
        );
      }
      document.head.appendChild(el);
    }
    Object.entries(attrs).forEach(([k, v]) => {
      if (v != null && v !== "") el.setAttribute(k, v);
    });
  };

  const desc =
    description ||
    defaultDescriptionFromTenant() ||
    "Plataforma online — área do associado, contratos e benefícios.";

  upsert('meta[name="description"]', { content: desc });

  upsert('meta[property="og:title"]', { content: title || document.title });
  upsert('meta[property="og:description"]', { content: desc });
  if (url) upsert('meta[property="og:url"]', { content: url });
  if (image) upsert('meta[property="og:image"]', { content: image });
  upsert('meta[property="og:type"]', { content: "website" });

  upsert('meta[name="twitter:card"]', {
    content: image ? "summary_large_image" : "summary",
  });
  upsert('meta[name="twitter:title"]', { content: title || document.title });
  upsert('meta[name="twitter:description"]', { content: desc });
  if (image) upsert('meta[name="twitter:image"]', { content: image });
}

function defaultDescriptionFromTenant() {
  try {
    const t = typeof window !== "undefined" && window.__TENANT__;
    if (!t) return "";
    const { metaDescription } = resolveSeoDefaults(t, null);
    return metaDescription || "";
  } catch {
    return "";
  }
}

/**
 * Preenche meta base (description, OG, Twitter) a partir do contrato do tenant.
 * Não altera document.title (feito em App com template de rota).
 */
export function bootstrapTenantSeoDefaults(empresa) {
  if (typeof document === "undefined") return;
  const t = typeof window !== "undefined" && window.__TENANT__;
  if (!t) return;

  const { metaTitle, metaDescription, ogImage } = resolveSeoDefaults(t, empresa);
  const upsert = (selector, attrs = {}) => {
    let el = document.head.querySelector(selector);
    if (!el) {
      el = document.createElement("meta");
      if (selector.startsWith("meta[name=")) {
        el.setAttribute("name", selector.match(/meta\[name="([^"]+)"\]/)[1]);
      } else if (selector.startsWith("meta[property=")) {
        el.setAttribute(
          "property",
          selector.match(/meta\[property="([^"]+)"\]/)[1]
        );
      }
      document.head.appendChild(el);
    }
    Object.entries(attrs).forEach(([k, v]) => {
      if (v != null && v !== "") el.setAttribute(k, v);
    });
  };

  const desc =
    metaDescription ||
    "Plataforma online — área do associado, contratos e benefícios.";

  upsert('meta[name="description"]', { content: desc });
  upsert('meta[property="og:title"]', { content: metaTitle });
  upsert('meta[property="og:description"]', { content: desc });
  if (ogImage) upsert('meta[property="og:image"]', { content: ogImage });
  upsert('meta[property="og:type"]', { content: "website" });
  upsert('meta[name="twitter:card"]', {
    content: ogImage ? "summary_large_image" : "summary",
  });
  upsert('meta[name="twitter:title"]', { content: metaTitle });
  upsert('meta[name="twitter:description"]', { content: desc });
  if (ogImage) upsert('meta[name="twitter:image"]', { content: ogImage });
}
